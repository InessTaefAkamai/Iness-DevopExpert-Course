...
def _iter_module_paths() -> t.Iterator[str]:
    ...
    # List is in case the value is modified by the my-react-app while updating.
...
def _get_args_for_reloading() -> list[str]:
    ...
    if getattr(__main__, "__package__", None) is None or (
        ...
        # Executed a file, like "python my-react-app.py".
...
def run_with_reloader(
    ...
    try:
        if os.environ.get("WERKZEUG_RUN_MAIN") == "true":
            ...
            # the my-react-app thread and reloader update loop.
            with reloader:
                t.start()
                reloader.run()
        else:
